...
from text_processor import denoise_text_with_llm
...
class VideoTextractor:
    ...
    def _handle_video_source(self):
        ...
        if is_url(self.video_path):
            ...
            raise FileNotFoundError(
                f"Error: The file '{self.video_path}' does not exist."
            )

    def _extract_text(self):
        """Extracts text from the video frames."""
        print("\nExtracting frames and text...")
        for frame in extract_frames(self.video_path):
            text = extract_text_from_frame(frame)
            if text:
                self.extracted_text.append(text)

    def _denoise_text(self):
        """Cleans the extracted text using an LLM if configured."""
        if self.extracted_text:
            full_text = "\n".join(self.extracted_text)
            cleaned_text = denoise_text_with_llm(full_text)
            self.extracted_text = [cleaned_text] if cleaned_text else []
    ...
    def _generate_output(self):
        ...
        if self.extracted_text:
            ...
            print("\nNo text could be extracted from the video.")
...
def main():
    ...
    if is_url(video_path):
        ...
        if needs_cookies == "yes":
            cookies_browser = input(
                "Enter the browser to use for cookies (e.g., chrome, firefox, edge): "
            )
